    ...
    function createParticles(){
      ...
      try{
        for(let i=0;i<50;i++){
          ...
          p.classList.add(s[Math.floor(Math.random()*s.length)]);
    ...
    function toggleMenu(){
      try{
        ...
        if(ovr)ovr.classList.toggle('active');
    ...
    ovr?.addEventListener('click',toggleMenu);
    ...
    try{
      obs=new IntersectionObserver(ent=>{
        ent.forEach(e=>{
    ...
    const emailElement=document.querySelector('.profile .value');
    if(emailElement){
